# CODE SNAPSHOT v:2026391AUT0041342227
...
📦 Zeskanowano fizycznie: 742 plików, 125 katalogów
```

## Structure

```plaintext
 ▣─┬ querypath-snapshot             [15.4 KiB]                      A:/A-JAN/WIN-DOCS/REPO_OWN_RUST/GIT 
...
2  ├──• README.md                   [ 2.1 KiB] [2026-38-7 23:40:32] ./README.md                         
   ├──┬ examples                    [ 5.5 KiB] [2026-39-1 00:17:24] ./examples/                         
3  │  ├──• demo.rs                  [ 4.5 KiB] [2026-39-1 00:41:28] ./examples/demo.rs                  
4  │  └──• demo_time.rs             [  1004 B] [2026-39-1 00:26:12] ./examples/demo_time.rs             
   ├──┬ src                         [ 6.4 KiB] [2026-38-7 22:59:14] ./src/                              
5  │  ├──• lib.rs                   [ 6.4 KiB] [2026-39-1 00:35:17] ./src/lib.rs                        
...
### [4] `./examples/demo_time.rs`

```rust
// ./examples/demo_time.rs
use std::time::SystemTime;
use temporal_fmt::Temporal;

fn main() {
	let now = SystemTime::now();
	let pattern = "WYYYWWDSSShhmmssttqq";

	let formatted = Temporal::format_system_time(now, pattern);

	println!("⏰ Test formatowania czasu (temporal-fmt v0.1.1):");
	println!(" ├─ Wzorzec (pattern): \"{}\"", pattern);
	println!(" └─ Wynik (formatted): {}\n", formatted);

	println!("🔍 Rozbicie tokenów dla wzorca \"{}\":", pattern);
	println!(" ├─ WYYY : Rok ISO (4 cyfry)");
	println!(" ├─ WW   : Numer tygodnia ISO (01-53)");
	println!(" ├─ SSS  : Meteorologiczna pora roku (AUT / WIN / SPR / SUM)");
	println!(" ├─ D    : Dzień tygodnia (1 = Mon ... 7 = Sun)");
	println!(" ├─ hh   : Godzina (00-23)");
	println!(" ├─ mm   : Minuta (00-59)");
	println!(" ├─ ss   : Sekunda (00-59)");
	println!(" ├─ tt   : Tercja / Tierce (1/60 s, 00-59)");
	println!(" └─ qq   : Kwadra / Quadra (1/3600 s, 00-59)");
}
```
